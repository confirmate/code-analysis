...
fun endpointsHaveRateLimiting(): QueryTree<Boolean> {
    return translationResult.allExtended<HttpEndpoint> {
        val result =
            QueryTree(
                value = it.rateLimiting != null,
                stringRepresentation =
                    if (it.rateLimiting != null) {
        ...
        if (result.value) {
            result.assume(
                ...
                "We assume that the rate limiting on endpoint ${it.name} has meaningful limits: ${it.rateLimiting?.maxRequests} per  ${it.rateLimiting?.timeWindowSeconds} seconds.\n\nTo validate this assumption, we should identify if this makes sense for the performance of our system and the expected load.",
            )
        } else {
            result
        }
