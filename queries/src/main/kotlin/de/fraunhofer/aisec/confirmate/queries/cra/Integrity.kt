...
fun secureHttpResponses(
    requiresEncryption: (HttpEndpointOperation) -> Boolean
): QueryTree<Boolean> =
    // We start with the HttpEndpointOperations as they represent the server-side handling of
    // requests
    translationResult.allExtended<HttpEndpointOperation>(requiresEncryption) {
        ...
        // We need a secure channel or the data must be used for some other integrity protection before sending
        secureChannel
    }
...
fun inTransitEncryptionEnabled(
    ...
    val secureHttpRequests = httpRequestsUseSecureTransport(requiresEncryption)
    // Let's just say that all responses need encryption.
    val secureHttpResponses = secureHttpResponses({ true } )

    val secureConfig = configurationHasEncryptionEnabled(encryptionEnabledCheck)
