...
import de.fraunhofer.aisec.cpg.graph.concepts.ontology.DatabaseOperation
import de.fraunhofer.aisec.cpg.graph.concepts.ontology.DatabaseQuery
import de.fraunhofer.aisec.cpg.graph.concepts.ontology.DatabaseStorage
...
fun dbHasTTLConfigured(): QueryTree<Boolean> {
    return tr.allExtended<DatabaseStorage> { dbStorage ->
        QueryTree(
...
/** Helper function to check if a [DatabaseStorage] has a retention policy configured. */
fun DatabaseStorage.hasTTLOrRetentionPeriod(): Boolean {
    if (this.timeToLiveSeconds != null || this.hasAutomatedCleanup == true) {
        return true
    }
...
fun dbStoredDataIsRead(
    isWriteQuery: (DatabaseQuery) -> Boolean,
    isReadQuery: (DatabaseQuery) -> Boolean,
): QueryTree<Boolean> {
    return tr.allExtended<DatabaseQuery>(
...
fun DatabaseQuery.parametersFlowToReadQuery(
    isReadQuery: (DatabaseQuery) -> Boolean
): QueryTree<Boolean> {
    val parameterFlows =
        this.parameters.map { param ->
            dataFlow(
                    ...
                    scope = Interprocedural(),
                    predicate = { node ->
                        node.overlays.filterIsInstance<DatabaseQuery>().any(isReadQuery)
                    },
                )
                .assume(
