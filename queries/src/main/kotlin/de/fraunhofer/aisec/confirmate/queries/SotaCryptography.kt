...
// Symmetric encryption algorithms, their modus, and recommended key lengths. There are also requirements on the IV, and interdependencies between accepted key lengths and modus.
fun Cipher.checkAES(): QueryTree<Boolean> {
    return listOf(::checkAesCCM, ::checkAesGCM, ::checkAesGcmSiv, ::checkAesCBC, ::checkAesCTR).map { it.invoke() }.mergeWithAny(this)
}

fun Cipher.isAES(): QueryTree<Boolean> {
    val algoAccepted = this.cipherName?.uppercase()?.contains("AES") == true
    val algoCheck = QueryTree(
        value = algoAccepted,
        stringRepresentation = if (algoAccepted) {
            "Algorithm is AES"
        } else {
...
fun Cipher.isModus(modus: String): QueryTree<Boolean> {
    ...
    val modusCheck = QueryTree(
        value = modusAccepted,
        stringRepresentation = if (modusAccepted) {
            "Modus is $modus"
        } else {
...
fun Cipher.keyIsBlockSize(): QueryTree<Boolean> {
    ...
    val keyIsBlockSizeCheck = QueryTree(
        value = keySizeIsBlockSize,
        stringRepresentation = if (keySizeIsBlockSize) {
...
fun Cipher.validKeyLength(keySizes: Collection<Int>): QueryTree<Boolean> {
    ...
    val validKeyLengthCheck = QueryTree(
        value = validKeyLength,
        stringRepresentation = if (validKeyLength) {
...
fun Cipher.validAuthTagSize(minimalAuthTagSize: Int): QueryTree<Boolean> {
    ...
    val tagSizeCheck = QueryTree(
        value = tagSizeAccepted,
        stringRepresentation = if (tagSizeAccepted) {
...
fun Cipher.validIvSize(minimalIvSize: Int): QueryTree<Boolean> {
    ...
    val ivSizeCheck = QueryTree(
        value = ivSizeAccepted,
        stringRepresentation = if (ivSizeAccepted) {
...
fun Cipher.isUniqueIv(): QueryTree<Boolean> {
    // TODO: We cannot check this statically, but it is a requirement that the IV is unique for each encryption operation with the same key.
    val ivIsUnique = true
    val ivIsUniqueCheck = QueryTree(
        value = ivIsUnique,
        stringRepresentation = if(ivIsUnique)"The IV is unique for each encryption operation with the same key" else "The IV is not unique for each encryption operation with the same key",
        node = this,
        operator = GenericQueryOperators.EVALUATE,
    )
...
fun Cipher.checkAesCCM(): QueryTree<Boolean> {
    ...
    // Note: The TR itself has no limitation on the key size for AES-CCM, but it references NIST SP 800-38C, which only defines AES-CCM for key sizes of 128 bits.
    ...
    val allChecks = listOf(algoCheck, modusCheck, keyIsBlockSizeCheck, validKeyLengthCheck, tagSizeCheck, ivLengthCheck, ivIsUniqueCheck)
...
fun Cipher.checkAesGcmSiv(): QueryTree<Boolean> {
    ...
    val allChecks = listOf(algoCheck, modusCheck, keyIsBlockSizeCheck, validKeyLengthCheck, tagSizeCheck, ivLengthCheck, ivIsUniqueCheck, ivIsRandomCheck)
    val allChecksPassed = allChecks.all { it.value }
...
fun Cipher.checkAesCBC(): QueryTree<Boolean> {
    ...
    val allChecks = listOf(algoCheck, modusCheck, keyIsBlockSizeCheck, validKeyLengthCheck, ivLengthCheck, ivIsRandomCheck)
    val allChecksPassed = allChecks.all { it.value }
...
fun Cipher.checkAesCTR(): QueryTree<Boolean> {
    ...
    val allChecks = listOf(algoCheck, modusCheck, keyIsBlockSizeCheck, validKeyLengthCheck, ivLengthCheck, ivIsUniqueCheck)
    val allChecksPassed = allChecks.all { it.value }
