...
fun loggingEnabledByDefault(relevantActivitiesSpecifier: ((Node) -> Boolean)): QueryTree<Boolean> {
    TODO()
}

/**
 * This query checks that there is an opt-out mechanism for logging. I.e., it must be possible to
 * disable logging via a specific user input.
 */
context(translationResult: TranslationResult)
fun loggingOptOut(relevantActivitiesSpecifier: ((Node) -> Boolean)): QueryTree<Boolean> {
    // Idea: There exists a user input which flows into a CDG parent (possible transitive) for each
