 ...
 * disable logging via a specific user input. How this input is provided for the TOE is specified by [inputConfiguringOptOut].
 */
context(translationResult: TranslationResult)
fun loggingOptOut(inputConfiguringOptOut: ((Node) -> Boolean)): QueryTree<Boolean> {
    ...
    return translationResult.allExtended<LogWrite> { logWrite ->
        // We have to make use of the underlying node here, as the LogWrite itself is not connected via the CDG
        val surroundingCDGNodeIsOptOut = logWrite.underlyingNode?.prevCDGEdges?.map { it.isOptOutCheck(inputConfiguringOptOut) } ?: emptyList()
        QueryTree(
            ...
            """
                ...
                Note that this is only necessary if the query fails.""".trimIndent(),
        )
    }
}

fun ControlDependence.isOptOutCheck(inputConfiguringOptOut: ((Node) -> Boolean)): QueryTree<Boolean> {
    val branchingNode = end as? BranchingNode ?: return QueryTree(
        value = false,
        stringRepresentation = "CDG parent is not a branching node",
        node = end,
        operator = GenericQueryOperators.EVALUATE,
    )
    val condition = branchingNode.branchedBy ?: return QueryTree(
        value = false,
