...
import de.fraunhofer.aisec.cpg.graph.concepts.autoGenerated.UnlockEncryptedDisk
...
fun securityConfigurationSinkSelector(node: Node): Boolean {
    val securityConceptsInCode = listOf(
        SecretOperation::class.java,
        SecretKeyFactory::class.java, Encryption::class.java, EncryptionOperation::class.java, CustomerKeyEncryption::class.java,
        CustomerKeyEncryption::class.java, AtRestEncryption::class.java, Attestation::class.java,
        AuthenticationOperation::class.java, Authorization::class.java,
        AutomaticUpdates::class.java,
        Certificate::class.java,
        AccessRestriction::class.java, CheckAccess::class.java,
        CipherOperation::class.java,
        CodeRepository::class.java,
        ContainerRegistry::class.java, ContainerOrchestration::class.java,
        CreateEncryptedDisk::class.java, CreateSecret::class,
        CryptographicHash::class.java, DDoSProtection::class.java,
        DynamicLoadingOperation::class.java,
        HttpServer::class.java,
        LoggingService::class.java, // instantiating a logging service can be configured with security relevant configs,
        ...
        DatabaseConnect::class.java, DatabaseService::class.java, DatabaseStorage::class.java,
        Firewall::class.java)
    return securityConceptsInCode.any {it == node.javaClass}
}

fun securityConfigurationSelector(node: Node): Boolean {
    val securityKeywords = listOf("secure", "ssl", "tls", "encrypt", "auth", "password", "token", "key")
    return securityKeywords.any { node.name.toString()?.lowercase()?.contains(it) == true }
}


context(translationResult: TranslationResult)
fun secureConfigAlwaysUsed(isSecurityConfiguration: (Node)-> Boolean = ::securityConfigurationSelector, isSecurityConfigurationSink: (Node)-> Boolean = ::securityConfigurationSinkSelector): QueryTree<Boolean> {
    // This query reads: If a configuration is security relevant, using the isSecurityConfiguration selector, we need to
    // ensure that it is used somewhere in the program to set a secure configuration to an operation. The analysis is a
    // May analysis, because the configuration data does not have to flow into such an operation in all cases, but logging
    // of security parameters is legitimate as well.
    return translationResult.allExtended<OverlayNode>( sel = {it -> (it is Configuration || it is ConfigurationOperation || it is ConfigurationOption) && isSecurityConfiguration(it)}) { node ->
        dataFlow(
            startNode = node,
            direction = Forward(GraphToFollow.DFG),
            type = May,
            predicate = {node ->
                isSecurityConfigurationSink(node)
            }
        )
    }
...
/**
 * Query to ensure that data going into security relevant operations is not coming from constants in the code, but configuration files or external data.
 */
context(translationResult: TranslationResult)
fun noNonConfigConstantsToSecureOperation(isSecurityConfiguration: (Node)-> Boolean = ::securityConfigurationSelector, isSecurityConfigurationSink: (Node)-> Boolean = ::securityConfigurationSinkSelector): QueryTree<Boolean> {
    return translationResult.allExtended<Node>( sel = {it -> securityConfigurationSinkSelector(it)}) { node ->
        dataFlow(
            startNode = node,
            direction = Backward(GraphToFollow.DFG),
            earlyTermination = {node -> node is Literal<*> },
            predicate = {node -> (node is Configuration || node is ConfigurationOperation || node is ConfigurationOption) || node.prevDFGEdges.isEmpty() }
        )
    }
}
...
fun secureValuesConfigured(): QueryTree<Boolean> {
    return translationResult.allExtended<ConfigurationOption> { confOp ->
        QueryTree<Boolean>(
            value = secureConfiguration.isSecureConfigValue(confOp.name.toString(), confOp.value.toString()),
            stringRepresentation = "The configuration of the value ${confOp.name} is either not security relevant or its value ${confOp.value} is considered to be secure by the default configuration.",
            node = null,
