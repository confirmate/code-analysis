      ...
      {#if resourceData}
        <div class="space-y-px">
          {#each Object.entries(resourceData) as [key, value]}
            <div class="rounded-md px-2.5 py-1.5 {key === popover.field ? 'bg-indigo-50' : 'hover:bg-gray-50'} transition-colors">
              <div class="text-[10px] font-medium uppercase tracking-wider {key === popover.field ? 'text-indigo-500' : 'text-gray-300'}">
                {key}
              </div>
              ...
              {:else}
                <div class="text-[13px] {key === popover.field ? 'text-indigo-900 font-medium' : 'text-gray-700'} mt-0.5">
                  {formatValue(value)}
                </div>
              {/if}
            </div>
