...
{:else if readonly}
  {#if display !== null}
    <span data-field="filled">{display}</span>
  {:else}
    <span
      data-field="missing"
...
{:else if editing}
  <input
    ...
    onblur={commit}
    onkeydown={onKey}
    class="inline-block min-w-[8ch] rounded-md px-2 py-0.5 text-[length:inherit] font-[inherit] text-gray-900 bg-gray-50 ring-1 ring-gray-200 focus:outline-none focus:bg-white focus:ring-gray-300 transition-all duration-150"
  />
{:else if display !== null}
  <button
    type="button"
    data-field="filled"
    class="group inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[length:inherit] font-medium text-gray-800 bg-white ring-1 ring-dashed ring-gray-200 hover:bg-indigo-50/60 hover:text-indigo-700 hover:ring-indigo-200 cursor-text transition-all duration-150"
    onclick={startEdit}
  >
    <span>{display}</span>
    <svg
...
{:else}
  <button
    type="button"
    data-field="missing"
    class="group inline-flex items-center gap-1 rounded-md px-2 py-0.5 bg-amber-50/80 text-amber-500 ring-1 ring-dashed ring-amber-300 text-[12px] font-medium hover:bg-amber-50 hover:text-amber-600 cursor-text transition-all duration-150"
    onclick={startEdit}
  >
