<script lang="ts">
  ...
  import { generateSegments, countFields } from '$lib/utils/templateSegments';
  ...
  const segments = $derived(generateSegments(data.evidences, manufacturer));
  const fieldCount = $derived(countFields(segments));
  const progressPercent = $derived(
    fieldCount.total > 0 ? Math.round((fieldCount.filled / fieldCount.total) * 100) : 0
  );

  let popoverEvidence: Evidence | null = $state(null);
  let popoverResourceType = $state('');
  let popoverField = $state('');
  let popoverAnchor: HTMLElement | null = $state(null);

  const activeEvidenceField = $derived(
    popoverEvidence ? `${popoverResourceType}.${popoverField}` : null
  );

  function openPopover(evidence: Evidence, resourceType: string, field: string, target: HTMLElement) {
    popoverEvidence = evidence;
    popoverResourceType = resourceType;
    popoverField = field;
    popoverAnchor = target;
  }

  function closePopover() {
    popoverEvidence = null;
    popoverAnchor = null;
  }
</script>
...
<PageHeader
  ...
  subtitle="Generate CRA-compliant technical documentation"
/>
    ...
    <div class="px-6 pb-5 border-t border-gray-100">
      <form class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4" autocomplete="off">
        <label class="flex flex-col gap-1.5">
          ...
          <input
            type="text"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-transparent"
        ...
        <label class="flex flex-col gap-1.5">
          <span class="text-sm font-medium text-gray-700">Security portal URL</span>
          <input
            type="url"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-transparent"
        ...
        <label class="flex flex-col gap-1.5">
          ...
          <input
            ...
            bind:value={manufacturer.model}
          />
        </label>

        <label class="flex flex-col gap-1.5">
          <span class="text-sm font-medium text-gray-700">Serial number / batch / device ID</span>
          <input
            type="text"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-transparent"
            placeholder="SN-12345"
            bind:value={manufacturer.serialNumber}
          />
        </label>
        ...
        <label class="flex flex-col gap-1.5">
          ...
          <input
            type="text"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-transparent"
            placeholder="Automatic update function"
            bind:value={manufacturer.automaticUpdateMethod}
          />
        </label>
        ...
        <label class="flex flex-col gap-1.5">
          ...
          <input
            type="text"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-transparent"
            placeholder="Factory reset / Secure wipe"
            bind:value={manufacturer.dataRemovalMethod}
          />
        </label>
        ...
        <label class="flex flex-col gap-1.5">
          ...
          <input
            type="text"
            class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:border-transparent"
            placeholder="Settings > Updates"
            bind:value={manufacturer.disableUpdatesPath}
          />
        </label>
        ...
        <label class="flex flex-col gap-1.5 sm:col-span-2">
          ...
          <input
            ...
            bind:value={manufacturer.integrationDocUrl}
          />
        </label>
      </form>
    </div>
...
<!-- Document -->
<div class="bg-white shadow-sm rounded-lg border border-gray-100 px-10 py-8">
  <DocumentRenderer lines={segments} onEvidenceClick={openPopover} {activeEvidenceField} />
</div>

<!-- Evidence Popover -->
<EvidencePopover
  evidence={popoverEvidence}
  resourceType={popoverResourceType}
  field={popoverField}
